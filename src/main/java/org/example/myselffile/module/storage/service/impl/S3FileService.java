...
public class S3FileService implements StorageStrategy {
    ...
    public UploadInfo getUploadInfo(String path, String fileName) {
        ...
        // formData.put("success_action_status", "200"); // 可选

        // 构造阿里云上传地址 (http://bucket.endpoint)
    ...
    public boolean createFolder(String path, String name) {
        String key = formatKey(path + "/" + name + "/");
        s3Client.putObject(PutObjectRequest.builder().bucket(bucketName).key(key).build(), RequestBody.empty());
        return true;
    }
